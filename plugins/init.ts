...
import type { CommonContent, CommonContentMenus, MenuUid } from '~/composables/use-common-content'
...
function initCommonContent() {
    ...
    return useAsyncData<CommonContent>('common_content', async () => {
        ...
        const menus = menusResponse.reduce((acc: Partial<CommonContentMenus>, current: MenuDocument) => {
...
function initHead(document?: PrismicDocument, alternateLinks?: AlternateLanguage[]) {
    ...
    const alternateLinksHead = alternateLinks?.map((alternateLink: AlternateLanguage) => {
        return {
            ...
            href: joinURL(runtimeConfig.public.baseURL, alternateLink.uid || alternateLink.lang),
        }
    })
